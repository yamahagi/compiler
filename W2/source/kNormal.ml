...
| Syntax.If(Syntax.LE(e1, e2), e3, e4) ->
  ...
  | Syntax.Let((x, t), e1, e2) ->
	(match List.assoc e1 !ref_list with
	|x1 -> 
		let e2', t2 = g (M.add x t env) e2 in
		(Let((x, t), Var x1, e2'), t2 )
	|exception _ ->
	ref_list := (e1,x)::(!ref_list);
(*
find e1 in reference list 
List.assoc¿OKif found Var 
Not found g env etc...
judge by exception
*)
(* add x,e1 to reference list*)
...
and print_list_pair l =
    ...
    | (a, b) :: xs -> (print_string a;
                       ...
                       print_string " ";
                       print_list_pair xs)
and  print_syntax e =  (* KÀµµ¬²½¥ë¡¼¥Á¥óËÜÂÎ (caml2html: knormal_g) *)
  match e with
  ...
  | Neg(e) ->
  	      		...
  	      		print_string e;
			print_string " "
  | Add(e1, e2) -> (* Â­¤·»»¤ÎKÀµµ¬²½ (caml2html: knormal_add) *)
  			print_string "Add ";
			print_string e1;
  			print_string " ";
			print_string e2;
  ...
  | Let((x, t), e1, e2) ->
  	print_string "LET ";
             ...
             print_string "IN  \n";
             print_syntax e2
  | Var(x) -> (* ³°ÉôÇÛÎó¤Î»²¾È (caml2html: knormal_extarray) *)
		print_string "VAR ";
                print_string x  
| LetRec({ name = (x, t); args = yts; body = e1 }, e2) ->
  		print_string "LETREC ";
